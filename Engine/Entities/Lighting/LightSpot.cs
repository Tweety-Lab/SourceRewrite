...
namespace SourceRewrite.Entities.Lighting
{
    ...
    public class LightSpot : PointEntity, ILight
    {
        ...
        public void ApplyToShader(Shader shader, int lightIndex)
        {
            ...
            shader.SetParameter($"{lightPrefix}.cutOff", MathF.Cos(Math.DegreesToRadians(InnerConeAngle)));
            shader.SetParameter($"{lightPrefix}.outerCutOff", MathF.Cos(Math.DegreesToRadians(OuterConeAngle)));
        }
