...
public class VMFFormat
{
    ...
    private VMFSide ProcessSide(ParentKey pkSide)
    {
        ...
        // Process the plane
        var plane = ProcessSidePlane(planeValue);

        // Process UV Axis
        var uAxis = ProcessUVAxis((string)pkSide.GetKeyValue("uaxis").Value);
        var vAxis = ProcessUVAxis((string)pkSide.GetKeyValue("vaxis").Value);
